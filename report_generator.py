...
def generate_participant_report(records: pd.DataFrame, output_dir: Path | str | None = None) -> Path:
    if records is None or records.empty:
        raise ValueError("No hay registros disponibles para generar el informe.")
    ...
    if output_dir is None:
        participant_folder = (
            ...
            / (safe_filename(participant.get("participant_id")) + "_" + safe_filename(participant.get("name")))
        )
    else:
    ...
    add_cover(document, participant, records)
    add_executive_summary(document, clinical_data)
    add_introduction_and_participant(document, participant, clinical_data)
    add_descriptive_summary(document, clinical_data)
    add_blood_pressure_section(document, clinical_data)
    add_wellbeing_section(document, clinical_data)
    add_moment_report_sections(document, clinical_data)
    add_scientific_context(document, clinical_data)
    add_temporal_evolution(document, clinical_data)
    add_data_quality(document, clinical_data)
    add_graphs_section(document, records, clinical_data, participant_folder)
    add_conclusions(document, clinical_data)

    document.core_properties.title = "Informe individual de bienestar y salud"
    document.core_properties.subject = PROJECT_NAME
    document.core_properties.author = AUTHOR
    document.core_properties.keywords = "bienestar, cáncer de mama, seguimiento longitudinal, salud"
